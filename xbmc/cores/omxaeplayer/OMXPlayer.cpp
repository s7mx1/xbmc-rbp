...
#include "../omxplayer/BitstreamConverter.h"
...
// ****************************************************************
void COMXAESelectionStreams::Clear(StreamType type, StreamSource source)
{
...
void COMXAEPlayer::GetAudioStreamLanguage(int iStream, CStdString &strLanguage)
{
  strLanguage = "";
  OMXAESelectionStream& s = m_SelectionStreams.Get(STREAM_AUDIO, iStream);
  if(s.language.length() > 0)
    strLanguage = s.language;
}

OMXAESelectionStream& COMXAESelectionStreams::Get(StreamType type, int index)
{
  CSingleLock lock(m_section);
  int count = -1;
...
bool COMXAESelectionStreams::Get(StreamType type, CDemuxStream::EFlags flag, OMXAESelectionStream& out)
{
  CSingleLock lock(m_section);
  for(int i=0;i<(int)m_Streams.size();i++)
  {
...
std::vector<OMXAESelectionStream> COMXAESelectionStreams::Get(StreamType type)
{
  std::vector<OMXAESelectionStream> streams;
...
static bool PredicateAudioPriority(const OMXAESelectionStream& lh, const OMXAESelectionStream& rh)
{
...
static bool PredicateSubtitlePriority(const OMXAESelectionStream& lh, const OMXAESelectionStream& rh)
{
...
static bool PredicateVideoPriority(const OMXAESelectionStream& lh, const OMXAESelectionStream& rh)
{
  PREDICATE_RETURN(lh.flags & CDemuxStream::FLAG_DEFAULT
                 , rh.flags & CDemuxStream::FLAG_DEFAULT);
  return false;
...
int COMXAESelectionStreams::IndexOf(StreamType type, int source, int id) const
{
  CSingleLock lock(m_section);
  int count = -1;
  for(int i=0;i<(int)m_Streams.size();i++)
...
int COMXAESelectionStreams::IndexOf(StreamType type, COMXAEPlayer& p) const
{
...
int COMXAESelectionStreams::Source(StreamSource source, std::string filename)
{
  CSingleLock lock(m_section);
  int index = source - 1;
  for(int i=0;i<(int)m_Streams.size();i++)
  {
    OMXAESelectionStream &s = m_Streams[i];
...
void COMXAESelectionStreams::Update(OMXAESelectionStream& s)
{
  CSingleLock lock(m_section);
  int index = IndexOf(s.type, s.source, s.id);
  if(index >= 0)
  {
    OMXAESelectionStream& o = Get(s.type, index);
...
void COMXAESelectionStreams::Update(CDVDInputStream* input, CDVDDemux* demuxer)
{
...
// ****************************************************************
COMXAEPlayer::COMXAEPlayer(IPlayerCallback &callback)
      : IPlayer(callback),
      CThread("COMXAEPlayer"),
      m_ready(true),
...
COMXAEPlayer::~COMXAEPlayer()
{
  CloseFile();
}

bool COMXAEPlayer::OpenFile(const CFileItem &file, const CPlayerOptions &options)
{
  try
  {
    CLog::Log(LOGNOTICE, "COMXAEPlayer: Opening: %s", file.GetPath().c_str());
...
bool COMXAEPlayer::CloseFile()
{
  CLog::Log(LOGDEBUG, "COMXAEPlayer::CloseFile");
  ...
  CLog::Log(LOGDEBUG, "COMXAEPlayer: waiting for threads to exit");
  ...
  CLog::Log(LOGDEBUG, "COMXAEPlayer: finished waiting");
...
bool COMXAEPlayer::IsPlaying() const
{
  return !m_bStop;
}

void COMXAEPlayer::OnStartup()
{
...
bool COMXAEPlayer::OpenInputStream()
{
  if(m_pInputStream)
...
retry:
  ...
  {
    CLog::Log(LOGERROR, "COMXAEPlayer::OpenInputStream - unable to create input stream for [%s]", m_filename.c_str());
    return false;
  }
  else
  ...
  if (!m_pInputStream->Open(m_filename.c_str(), m_mimetype))
  {
    ...
    {
      CLog::Log(LOGERROR, "COMXAEPlayer::OpenInputStream - failed to open [%s] as DVD ISO, trying Bluray", m_filename.c_str());
    ...
    CLog::Log(LOGERROR, "COMXAEPlayer::OpenInputStream - error opening [%s]", m_filename.c_str());
    return false;
  }
  ...
  {
    CLog::Log(LOGINFO, "COMXAEPlayer::OpenInputStream - DVD/BD not supported");
    return false;
  }
...
bool COMXAEPlayer::OpenDemuxStream()
{
  if(m_pDemuxer)
...
void COMXAEPlayer::OpenDefaultStreams()
{
  //int  count;
  OMXAESelectionStreams streams;
  bool valid;

  // open video stream
  streams = m_SelectionStreams.Get(STREAM_VIDEO, PredicateVideoPriority);
  valid   = false;
  for(OMXAESelectionStreams::iterator it = streams.begin(); it != streams.end() && !valid; ++it)
  {
...
bool COMXAEPlayer::ReadPacket(DemuxPacket*& packet, CDemuxStream*& stream)
{
...
bool COMXAEPlayer::IsValidStream(COMXAECurrentStream& stream)
{
  if(stream.id<0)
...
bool COMXAEPlayer::IsBetterStream(COMXAECurrentStream& current, CDemuxStream* stream)
{
...
void COMXAEPlayer::Process()
{
  ...
  //CLog::Log(LOGDEBUG, "COMXAEPlayer: Thread started");
  ...
  try
  {
    ...
    if(m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
    {
      CLog::Log(LOGNOTICE, "OMXAEPlayer: playing a dvd with menu's");
  ...
  {
    CLog::Log(LOGERROR, "COMXAEPlayer::Process: Exception thrown");
  }
}

void COMXAEPlayer::ProcessPacket(CDemuxStream* pStream, DemuxPacket* pPacket)
{
...
void COMXAEPlayer::ProcessAudioData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
...
void COMXAEPlayer::ProcessVideoData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
...
void COMXAEPlayer::ProcessSubData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
...
bool COMXAEPlayer::GetCachingTimes(double& level, double& delay, double& offset)
{
...
void COMXAEPlayer::HandlePlaySpeed()
{
  ...
  if(GetPlaySpeed() != DVD_PLAYSPEED_NORMAL && GetPlaySpeed() != DVD_PLAYSPEED_PAUSE)
  {
          ...
          &&  m_SpeedState.lasttime != GetTime())
    {
      ...
      {
        CLog::Log(LOGDEBUG, "COMXAEPlayer::Process - Seeking to catch up");
...
bool COMXAEPlayer::CheckStartCaching(COMXAECurrentStream& current)
{
...
bool COMXAEPlayer::CheckPlayerInit(COMXAECurrentStream& current, unsigned int source)
{
  if(current.inited)
    return false;
  ...
  if (current.dts != DVD_NOPTS_VALUE)
  {
    ...
    if(starttime > 0 && setclock)
    {
      if(starttime > DVD_SEC_TO_TIME(2))
        CLog::Log(LOGWARNING, "COMXAEPlayer::CheckPlayerInit(%d) - Ignoring too large delay of %f", source, starttime);
      else
...
void COMXAEPlayer::UpdateCorrection(DemuxPacket* pkt, double correction)
{
  if(pkt->dts != DVD_NOPTS_VALUE) pkt->dts -= correction;
  if(pkt->pts != DVD_NOPTS_VALUE) pkt->pts -= correction;
}

void COMXAEPlayer::UpdateTimestamps(COMXAECurrentStream& current, DemuxPacket* pPacket)
{
...
void COMXAEPlayer::UpdateLimits(double& minimum, double& maximum, double dts)
{
...
void COMXAEPlayer::CheckContinuity(COMXAECurrentStream& current, DemuxPacket* pPacket)
{
  if (m_playSpeed < DVD_PLAYSPEED_PAUSE)
    return;
  ...
  {
    CLog::Log(LOGDEBUG, "COMXAEPlayer::CheckContinuity - resync forward :%d, prev:%f, curr:%f, diff:%f"
  ...
  {
    CLog::Log(LOGDEBUG, "COMXAEPlayer::CheckContinuity - resync backward :%d, prev:%f, curr:%f, diff:%f"
  ...
  {
    CLog::Log(LOGDEBUG, "COMXAEPlayer::CheckContinuity - wrapback :%d, prev:%f, curr:%f, diff:%f"
                            , current.type, current.dts, pPacket->dts, pPacket->dts - current.dts);
  }
...
bool COMXAEPlayer::CheckSceneSkip(COMXAECurrentStream& current)
{
  if(!m_Edl.HasCut())
    return false;
...
void COMXAEPlayer::CheckAutoSceneSkip()
{
  if(!m_Edl.HasCut())
    return;
...
void COMXAEPlayer::SynchronizeDemuxer(unsigned int timeout)
{
  if(IsCurrentThread())
    return;
...
void COMXAEPlayer::SynchronizePlayers(unsigned int sources)
{
...
void COMXAEPlayer::SendPlayerMessage(CDVDMsg* pMsg, unsigned int target)
{
...
void COMXAEPlayer::OnExit()
{
  try
  {
    CLog::Log(LOGNOTICE, "COMXAEPlayer::OnExit()");
    ...
    // close each stream
    if (!m_bAbortRequest) CLog::Log(LOGNOTICE, "OMXAEPlayer: eof, waiting for queues to empty");
    if (m_CurrentAudio.id >= 0)
    {
      CLog::Log(LOGNOTICE, "OMXAEPlayer: closing audio stream");
      CloseAudioStream(!m_bAbortRequest);
    }
    if (m_CurrentVideo.id >= 0)
    {
      CLog::Log(LOGNOTICE, "OMXAEPlayer: closing video stream");
      CloseVideoStream(!m_bAbortRequest);
    }
    if (m_CurrentSubtitle.id >= 0)
    {
      CLog::Log(LOGNOTICE, "OMXAEPlayer: closing subtitle stream");
      CloseSubtitleStream(!m_bAbortRequest);
    }
    /*
    if (m_CurrentTeletext.id >= 0)
    {
      CLog::Log(LOGNOTICE, "OMXAEPlayer: closing teletext stream");
    ...
    */
    ...
    if (m_pDemuxer)
    {
      CLog::Log(LOGNOTICE, "COMXAEPlayer::OnExit() deleting demuxer");
      delete m_pDemuxer;
    }
    m_pDemuxer = NULL;
    ...
    if (m_pSubtitleDemuxer)
    {
      CLog::Log(LOGNOTICE, "COMXAEPlayer::OnExit() deleting subtitle demuxer");
    ...
    if (m_pInputStream)
    {
      CLog::Log(LOGNOTICE, "COMXAEPlayer::OnExit() deleting input stream");
...
void COMXAEPlayer::HandleMessages()
{
  CDVDMsg* pMsg;
  OMXStreamLock lock(this);
  ...
  while (m_messenger.Get(&pMsg, 0) == MSGQ_OK)
  {
    ...
    try
    {
      ...
      else if (pMsg->IsType(CDVDMsg::PLAYER_SET_AUDIOSTREAM))
      {
        ...
        OMXAESelectionStream& st = m_SelectionStreams.Get(STREAM_AUDIO, pMsg2->GetStreamId());
      ...
      else if (pMsg->IsType(CDVDMsg::PLAYER_SET_SUBTITLESTREAM))
      {
        ...
        OMXAESelectionStream& st = m_SelectionStreams.Get(STREAM_SUBTITLE, pMsg2->GetStreamId());
        if(st.source != STREAM_SOURCE_NONE)
        {
          if(st.source == STREAM_SOURCE_NAV && m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
          {
      ...
      else if (pMsg->IsType(CDVDMsg::PLAYER_STARTED))
      {
        ...
        CLog::Log(LOGDEBUG, "COMXAEPlayer::HandleMessages - player started %d", player);
      }
    }
    catch (...)
    {
...
void COMXAEPlayer::SetCaching(ECacheState state)
{
  ...
  CLog::Log(LOGDEBUG, "COMXAEPlayer::SetCaching - caching state %d", state);
...
void COMXAEPlayer::SetPlaySpeed(int speed)
{
...
bool COMXAEPlayer::IsPaused() const
{
  return (m_playSpeed == DVD_PLAYSPEED_PAUSE) || m_caching == CACHESTATE_FULL;
}

bool COMXAEPlayer::HasVideo() const
{
...
bool COMXAEPlayer::HasAudio() const
{
  return m_SelectionStreams.Count(STREAM_AUDIO) > 0 ? true : false;
}

bool COMXAEPlayer::IsPassthrough() const
{ 
  return m_player_audio.IsPassthrough();
}

bool COMXAEPlayer::CanSeek()
{
  return GetTotalTime() > 0;
}

void COMXAEPlayer::Seek(bool bPlus, bool bLargeStep)
{
...
bool COMXAEPlayer::SeekScene(bool bPlus)
{
...
void COMXAEPlayer::GetAudioInfo(CStdString &strAudioInfo)
{
...
void COMXAEPlayer::GetVideoInfo(CStdString &strVideoInfo)
{
  { CSingleLock lock(m_StateSection);
...
void COMXAEPlayer::GetGeneralInfo(CStdString& strGeneralInfo)
{
  if (!m_bStop)
  {
    ...
    strGeneralInfo.Format("C( ad:% 6.3f, a/v:% 6.3f%s, dcpu:%2i%% acpu:%2i%% vcpu:%2i%%%s, omx vb:%8d ad:% 6.3f )"
                         ...
                         , strBuf.c_str()
                         , m_player_video.GetDelay()
                         , 0 );
  }
}

void COMXAEPlayer::SeekPercentage(float fPercent)
{
...
float COMXAEPlayer::GetPercentage()
{
  int64_t iTotalTime = GetTotalTimeInMsec();
...
int COMXAEPlayer::GetSubtitle()
{
  return m_SelectionStreams.IndexOf(STREAM_SUBTITLE, *this);
}

void COMXAEPlayer::GetSubtitleName(int iStream, CStdString &strStreamName)
{
  strStreamName = "";
  OMXAESelectionStream& s = m_SelectionStreams.Get(STREAM_SUBTITLE, iStream);
  if(s.name.length() > 0)
...
void COMXAEPlayer::GetSubtitleLanguage(int iStream, CStdString &strStreamLang)
{
  OMXAESelectionStream& s = m_SelectionStreams.Get(STREAM_SUBTITLE, iStream);
...
void COMXAEPlayer::SetSubtitle(int iStream)
{
  m_messenger.Put(new CDVDMsgPlayerSetSubtitleStream(iStream));
}

bool COMXAEPlayer::GetSubtitleVisible()
{
  if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
  {
    CDVDInputStreamNavigator* pStream = (CDVDInputStreamNavigator*)m_pInputStream;
...
void COMXAEPlayer::SetSubtitleVisible(bool bVisible)
{
...
int COMXAEPlayer::GetAudioStreamCount()
{
  OMXStreamLock lock(this);
  m_SelectionStreams.Update(m_pInputStream, m_pDemuxer);
...
int COMXAEPlayer::GetAudioStream()
{
  return m_SelectionStreams.IndexOf(STREAM_AUDIO, *this);
}

void COMXAEPlayer::GetAudioStreamName(int iStream, CStdString &strStreamName)
{
  strStreamName = "";
  OMXAESelectionStream& s = m_SelectionStreams.Get(STREAM_AUDIO, iStream);
  if(s.name.length() > 0)
...
void COMXAEPlayer::SetAudioStream(int iStream)
{
  m_messenger.Put(new CDVDMsgPlayerSetAudioStream(iStream));
  SynchronizeDemuxer(100);
}

void COMXAEPlayer::SeekTime(int64_t iTime)
{
...
int64_t COMXAEPlayer::GetTime()
{
  ...
  //printf("COMXAEPlayer::GetTime %Lf offset %Lf %Lf\n", m_State.time, offset, m_av_clock.GetClock());
  return llrint(m_State.time + DVD_TIME_TO_MSEC(offset));
}

// return length in msec
int64_t COMXAEPlayer::GetTotalTimeInMsec()
{
  CSingleLock lock(m_StateSection);
...
int COMXAEPlayer::GetTotalTime()
{
  return (int)(GetTotalTimeInMsec() / 1000);
}

void COMXAEPlayer::ToFFRW(int iSpeed)
{
...
bool COMXAEPlayer::OpenAudioStream(int iStream, int source)
{
...
bool COMXAEPlayer::OpenVideoStream(int iStream, int source)
{
...
bool COMXAEPlayer::OpenSubtitleStream(int iStream, int source)
{
  ...
  if(STREAM_SOURCE_MASK(source) == STREAM_SOURCE_DEMUX_SUB)
  {
    ...
    if(index < 0)
      return false;
    OMXAESelectionStream st = m_SelectionStreams.Get(STREAM_SUBTITLE, index);
...
bool COMXAEPlayer::CloseAudioStream(bool bWaitForBuffers)
{
  if (m_CurrentAudio.id < 0)
    return false;
...
bool COMXAEPlayer::CloseVideoStream(bool bWaitForBuffers)
{
  if (m_CurrentVideo.id < 0)
    return false;
...
bool COMXAEPlayer::CloseSubtitleStream(bool bKeepOverlays)
{
  if (m_CurrentSubtitle.id < 0)
    return false;
...
void COMXAEPlayer::FlushBuffers(bool queued, double pts, bool accurate)
{
  double startpts;
  if(accurate)
    startpts = pts;
...
int COMXAEPlayer::OnDVDNavResult(void* pData, int iMessage)
{
...
bool COMXAEPlayer::OnAction(const CAction &action)
{
...
bool COMXAEPlayer::IsInMenu() const
{
...
bool COMXAEPlayer::GetCurrentSubtitle(CStdString& strSubtitle)
{
  if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
    return false;
...
CStdString COMXAEPlayer::GetPlayerState()
{
  CSingleLock lock(m_StateSection);
  return m_State.player_state;
}

bool COMXAEPlayer::SetPlayerState(CStdString state)
{
  m_messenger.Put(new CDVDMsgPlayerSetState(state));
  return true;
}

int COMXAEPlayer::GetChapterCount()
{
  CSingleLock lock(m_StateSection);
  return m_State.chapter_count;
}

int COMXAEPlayer::GetChapter()
{
  CSingleLock lock(m_StateSection);
  return m_State.chapter;
}

void COMXAEPlayer::GetChapterName(CStdString& strChapterName)
{
  CSingleLock lock(m_StateSection);
  strChapterName = m_State.chapter_name;
}

int COMXAEPlayer::SeekChapter(int iChapter)
{
  if (GetChapterCount() > 0)
  {
    if (iChapter < 0)
...
int COMXAEPlayer::AddSubtitle(const CStdString& strSubPath)
{
  return AddSubtitleFile(strSubPath);
}

int COMXAEPlayer::GetCacheLevel() const
{
  CSingleLock lock(m_StateSection);
  return (int)(m_State.cache_level * 100);
}

double COMXAEPlayer::GetQueueTime()
{
  int a = m_player_audio.GetLevel();
  int v = m_player_video.GetLevel();
  return max(a, v) * 8000.0 / 100;
}

int COMXAEPlayer::GetAudioBitrate()
{
  return m_player_audio.GetAudioBitrate();
}

int COMXAEPlayer::GetVideoBitrate()
{
  return m_player_video.GetVideoBitrate();
}

int COMXAEPlayer::GetSourceBitrate()
{
  if (m_pInputStream)
...
int COMXAEPlayer::AddSubtitleFile(const std::string& filename, const std::string& subfilename, CDemuxStream::EFlags flags)
{
...
void COMXAEPlayer::UpdatePlayState(double timeout)
{
  if(m_State.timestamp != 0
...
void COMXAEPlayer::UpdateApplication(double timeout)
{
...
bool COMXAEPlayer::CanRecord()
{
  CSingleLock lock(m_StateSection);
  return m_State.canrecord;
}

bool COMXAEPlayer::IsRecording()
{
  CSingleLock lock(m_StateSection);
  return m_State.recording;
}

bool COMXAEPlayer::Record(bool bOnOff)
{
  if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_TV))
  {
...
CStdString COMXAEPlayer::GetAudioCodecName()
{
  CStdString retVal;
  if (m_pDemuxer && (m_CurrentAudio.id != -1))
...
CStdString COMXAEPlayer::GetVideoCodecName()
{
  CStdString retVal;
...
int COMXAEPlayer::GetPictureWidth()
{
  if (m_pDemuxer && (m_CurrentVideo.id != -1))
  {
...
int COMXAEPlayer::GetPictureHeight()
{
  if (m_pDemuxer && (m_CurrentVideo.id != -1))
  {
    CDemuxStreamVideo* stream = static_cast<CDemuxStreamVideo*>(m_pDemuxer->GetStream(m_CurrentVideo.id));
...
bool COMXAEPlayer::GetStreamDetails(CStreamDetails &details)
{
  if (m_pDemuxer)
  {
...
CStdString COMXAEPlayer::GetPlayingTitle()
{
  return "";
}

void COMXAEPlayer::GetVideoRect(CRect& SrcRect, CRect& DestRect)
{
  g_renderManager.GetVideoRect(SrcRect, DestRect);
}

void COMXAEPlayer::SetVideoRect(const CRect &SrcRect, const CRect &DestRect)
{
  m_player_video.SetVideoRect(SrcRect, DestRect);
}

//void COMXAEPlayer::SetVolume(float fVolume)
//{
//  m_current_volume = fVolume;
//  m_change_volume = true;
//}

void COMXAEPlayer::Update(bool bPauseDrawing)
{
  g_renderManager.Update(bPauseDrawing);
}

void COMXAEPlayer::GetVideoAspectRatio(float &fAR)
{

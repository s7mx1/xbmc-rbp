...
public:
  ...
  COMXAECurrentStream(StreamType t, int i)
    : type(t)
    , player(i)
  {
    Clear();
...
typedef struct
{
  ...
  std::string  codec;
  int          channels;
} OMXAESelectionStream;

typedef std::vector<OMXAESelectionStream> OMXAESelectionStreams;

class COMXAESelectionStreams
{
  CCriticalSection m_section;
  OMXAESelectionStream  m_invalid;
public:
  COMXAESelectionStreams()
  {
    m_invalid.id = -1;
    m_invalid.source = STREAM_SOURCE_NONE;
    m_invalid.type = STREAM_NONE;
  }
  std::vector<OMXAESelectionStream> m_Streams;

  int              IndexOf (StreamType type, int source, int id) const;
  int              IndexOf (StreamType type, COMXAEPlayer& p) const;
  int              Count   (StreamType type) const { return IndexOf(type, STREAM_SOURCE_NONE, -1) + 1; }
  OMXAESelectionStream& Get     (StreamType type, int index);
  bool             Get     (StreamType type, CDemuxStream::EFlags flag, OMXAESelectionStream& out);

  OMXAESelectionStreams Get(StreamType type);
  template<typename Compare> OMXAESelectionStreams Get(StreamType type, Compare compare)
  {
    OMXAESelectionStreams streams = Get(type);
  ...
  void             Update  (OMXAESelectionStream& s);
  void             Update  (CDVDInputStream* input, CDVDDemux* demuxer);
};


class COMXAEPlayer : public IPlayer, public CThread, public IDVDPlayer
{
public:

  COMXAEPlayer(IPlayerCallback &callback);
  virtual ~COMXAEPlayer();
  

  virtual bool  IsValidStream(COMXAECurrentStream& stream);
  virtual bool  IsBetterStream(COMXAECurrentStream& current, CDemuxStream* stream);
  ...
  virtual bool  CheckPlayerInit(COMXAECurrentStream& current, unsigned int source);
  virtual void  UpdateCorrection(DemuxPacket* pkt, double correction);
  virtual void  UpdateTimestamps(COMXAECurrentStream& current, DemuxPacket* pPacket);
  virtual void  UpdateLimits(double& minimum, double& maximum, double dts);
  virtual bool  CheckSceneSkip(COMXAECurrentStream& current);
  virtual void  CheckAutoSceneSkip();
  virtual void  CheckContinuity(COMXAECurrentStream& current, DemuxPacket* pPacket);
  ...
  virtual void  SetDynamicRangeCompression(long drc)              {m_player_audio.SetDynamicRangeCompression(drc);}
  ...
  bool    CheckStartCaching(COMXAECurrentStream& current);
...
protected:
  friend class COMXAESelectionStreams;

  class OMXStreamLock : public CSingleLock
  {
  public:
    inline OMXStreamLock(COMXAEPlayer* comxplayer) : CSingleLock(comxplayer->m_critStreamSection) {}
  };
  ...
  COMXAESelectionStreams  m_SelectionStreams;
  std::string           m_mimetype;
  COMXAECurrentStream     m_CurrentAudio;
  COMXAECurrentStream     m_CurrentVideo;
  COMXAECurrentStream     m_CurrentSubtitle;
  COMXAECurrentStream     m_CurrentTeletext;
...
private:
  ...
  CDVDPlayerAudio         m_player_audio;
